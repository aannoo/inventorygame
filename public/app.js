...
let state=null,source=null,selection={desc:null,condition:null},questionId=null,offset=0,soundOn=localStorage.getItem('im-sound')!=='off',busy=false,renderedKey=null,joinDraft='';
...
function patchDynamic(){
  ...
  if(state.phase==='question'){
    document.querySelectorAll('.choice').forEach(button=>{
      const selected=selection[button.dataset.kind]===Number(button.dataset.index);
      button.classList.toggle('selected',selected);button.setAttribute('aria-pressed',String(selected));
      button.disabled=!!state.me?.answered;
    });
    const lock=document.querySelector('[data-action="answer"]');
    if(lock){lock.disabled=!!state.me?.answered||selection.desc===null||selection.condition===null;lock.textContent=state.me?.answered?'ANSWER LOCKED ✓':'LOCK IN REPORT ⚡';}
  }
  updateTimer();
}
...
function reviewCard(x){
  return `<div class="review"><img src="${x.image}" alt="Review photo"><div><b>${esc(x.room)} · ${esc(x.item)}</b>${x.scenario?`<p><b>Scenario facts:</b> ${esc(x.scenario)}</p>`:''}<p><b>Your ${esc(x.descLabel)}:</b> ${esc(x.chosenDesc)}</p><p><b>Your ${esc(x.conditionLabel)}:</b> ${esc(x.chosenCondition)}</p><p><b>Correct:</b> ${esc(x.correctDesc)} / ${esc(x.correctCondition)}</p><small>${esc(x.explain)}</small></div></div>`;
}
...
function render(){
  ...
  if(selectionKey!==questionId){selection={desc:null,condition:null};questionId=selectionKey;}
  const key=`${s.matchId}:${s.phase}:${s.round}`;
  const previousKey=renderedKey;
  if(key===renderedKey && (s.phase==='question'||s.phase==='poo')){patchDynamic();return;}
  const header=brand(`<span class="pill">${s.solo?'SOLO':'ROOM '+s.code}</span><button class="mute" data-action="sound" aria-label="${soundOn?'Mute sound':'Unmute sound'}">${soundOn?'🔊':'🔇'}</button>`)+(s.phase==='question'||s.phase==='feedback'||s.phase==='poo'?gameActions(s):'');
  if(s.phase==='lobby'){
  ...
  } else if(s.phase==='question'||s.phase==='feedback'){
    const q=s.question,feedback=s.phase==='feedback',mine=s.roundResults?.find(x=>x.id===s.me?.id),correct=mine?.desc&&mine?.cond;
    const line=correct?goodLines[(s.round-1)%goodLines.length]:badLines[(s.round-1)%badLines.length];
    app.innerHTML=header+`<section class="screen"><div class="topline"><span class="pill">ROUND ${s.round}/${s.total}</span><span class="pill">${feedback?'📣 VERDICT':'⏱ <span id="timer">18.0</span>s'}</span></div><div class="progress"><div id="timebar"></div></div>${q.scenario?`<div class="scenario"><b>SCENARIO FACTS</b><p>${esc(q.scenario)}</p></div>`:''}<div class="photo-wrap"><img src="${q.image}" alt="Property inspection photo"><span class="room-tag">${q.room}</span></div><h2 class="question-title">${esc(q.prompt)}</h2><p class="small coach">${q.scenario?'Use only the stated scenario facts and visible photo evidence.':'Clerk cue: room + item; colour, material, item; then severity, defect, location.'}</p><div class="label">${esc(q.descLabel)}</div><div class="choice-list">${q.descriptions.map((value,i)=>choice('desc',i,value,feedback,q.correctDesc)).join('')}</div><div class="label">${esc(q.conditionLabel)}</div><div class="choice-list">${q.conditions.map((value,i)=>choice('condition',i,value,feedback,q.correctCondition)).join('')}</div>${!feedback?(s.me?.answered?'<div class="feedback good">Answer locked. The others are still squinting.</div>':`<div class="buttons"><button class="button" data-action="answer" ${selection.desc===null||selection.condition===null?'disabled':''}>LOCK IN REPORT ⚡</button></div>`):`<div class="feedback ${correct?'good':''}">${correct?'+'+mine.points+' POINTS · ':mine?.points?`+${mine.points} POINTS · `:'0 POINTS · '}${line}</div><div class="explain"><b>Clerk’s note:</b> ${q.explain}</div>`}${scoreList(s.players)}</section>`;
  } else if(s.phase==='poo'){
    ...
    app.innerHTML=header+`<section class="screen"><div class="winner">🏆 ${tied.length>1?`${tied.map(p=>esc(p.name)).join(' & ')} TIE!`:`${esc(winner?.name||'The toilet')} WINS!`}</div><h2>${s.solo?'Inspection complete':'Final leaderboard'}</h2><p class="small">${s.solo?`You scored ${winner?.score||0} points. The clipboard has seen worse.`:`You finished #${me}. The clipboard remembers.`}</p>${scoreList(s.players)}${s.review?.length?`<h2>Missed evidence · ${s.review.length}</h2>${s.review.map(reviewCard).join('')}`:'<p class="note">No missed evidence. Annoyingly competent.</p>'}<div class="buttons">${s.host?`<button class="button" data-action="start">🔁 PLAY AGAIN</button>`:''}<button class="button secondary" data-action="leave">🏠 MAIN MENU</button></div></section>`;
  }
  renderedKey=key;
  updateTimer();
  if(previousKey!==key){
...
function choice(kind,i,value,feedback,correctIndex){
  const selected=feedback?state.me?.answer?.[kind]===i:selection[kind]===i;
  const verdict=feedback?(i===correctIndex?'correct':selected?'wrong':''):'';
  return `<button class="choice ${selected?'selected':''} ${verdict}" data-action="choice" data-kind="${kind}" data-index="${i}" aria-pressed="${selected}" ${feedback||state.me?.answered?'disabled':''}><b>${String.fromCharCode(65+i)}</b>${esc(value)}${feedback&&i===correctIndex?' ✓ Correct':feedback&&selected?' ✗ Your answer':''}</button>`;
}
function updateTimer(){
  ...
  const bar=document.querySelector('#timebar');if(bar)bar.style.width=`${remaining/(state.phase==='poo'?7000:state.phase==='feedback'?4500:18000)*100}%`;
}
function connect(){
...
document.addEventListener('click',async event=>{
  ...
  if(action==='choice'){selection[el.dataset.kind]=Number(el.dataset.index);sound(430);render();return;}
  if(action==='push'){
  ...
  busy=true;
  try{
    if(action==='solo'||action==='create'){
      savedIdentity();const result=await api('create',{solo:action==='solo',name,avatar});session={code:result.code,playerId:result.playerId};localStorage.setItem('im-session',JSON.stringify(session));state=result.state;connect();render();
    } else if(action==='join'){
    ...
    else if(action==='answer'){if(selection.desc===null||selection.condition===null)return;await api('answer',{...session,matchId:state.matchId,turnId:state.turnId,desc:selection.desc,condition:selection.condition});sound(700,.12);}
