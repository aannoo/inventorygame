...
function home(){
  ...
  app.innerHTML=brand()+`<section class="screen home-screen"><div class="hero"><div class="hero-copy"><div class="eyebrow">The inspection arcade · real property photos</div><h1>Spot it.<br>Report it.<br>Win.</h1><p>Read the room, catch the marks, and write the report before the clock runs out.</p></div><div class="hero-photos" aria-hidden="true"><img src="/assets/photos/23.webp" alt=""><img src="/assets/photos/38.webp" alt=""><img src="/assets/photos/16.webp" alt=""></div></div><div class="setup-card"><label class="label" for="name">Your clerk name</label><input id="name" class="field" maxlength="20" autocomplete="nickname" placeholder="Enter a name" value="${esc(name)}"><div class="label">Choose your clerk</div><div class="avatar-grid">${clerks.map(([id,label])=>`<button class="avatar-choice ${avatar===id?'selected':''}" data-action="avatar" data-avatar="${id}" aria-pressed="${avatar===id}"><img src="/assets/clerks/${id}.webp" alt=""><span>${label}</span></button>`).join('')}</div></div><h2 class="section-heading">Choose a game</h2><div class="mode-grid"><button class="mode-card" data-action="solo"><img src="/assets/photos/33.webp" alt=""><span class="mode-copy"><strong>⚡ Solo sprint</strong><small>12 fast rounds · beat your score</small></span><span class="mode-arrow">→</span></button><button class="mode-card" data-action="create"><img src="/assets/photos/14.webp" alt=""><span class="mode-copy"><strong>🏆 Live showdown</strong><small>Invite friends · race on every report</small></span><span class="mode-arrow">→</span></button><button class="mode-card" data-action="create-puzzle"><img src="/assets/photos/30.webp" alt=""><span class="mode-copy"><strong>🧩 Puzzle crew</strong><small>Build 3 reports together · no timer</small></span><span class="mode-arrow">→</span></button></div><div class="join-card"><label class="label" for="join-code">Have a room code?</label><div class="join-row"><input id="join-code" class="field" inputmode="latin" maxlength="6" autocapitalize="characters" placeholder="6-character code" value="${esc(joinDraft)}"><button class="button pink" data-action="join">Join</button></div><p class="small">Open the invite link first so everyone reaches the same game server.</p></div></section>`;
}
...
function render(){
  ...
  const s=state;
  if(s.phase==='feedback'&&photoViewer.open)photoViewer.close();
  const selectionKey=s.question?`${s.matchId}:${s.turnId}`:null;
  if(selectionKey!==questionId){selection={desc:null,condition:null,slots:[]};questionId=selectionKey;if(photoViewer.open)photoViewer.close();}
...
document.addEventListener('click',async event=>{
  ...
  if(action==='avatar'){avatar=el.dataset.avatar;document.querySelectorAll('.avatar-choice').forEach(x=>{const selected=x.dataset.avatar===avatar;x.classList.toggle('selected',selected);x.setAttribute('aria-pressed',String(selected));});sound(520);return;}
  if(action==='inspect'){const image=photoViewer.querySelector('img');image.src=el.dataset.image;image.alt=el.dataset.alt||'Enlarged inspection photo';photoViewer.showModal();return;}
